...
const registerCliente = async(req, res) => {
    ...
    const saveClient = await insertCliente(
        cli.cedula, cli.name,
        cli.email, cli.edad,
        cli.gen, cli.tel, cli.medio
    );
    if (saveClient === 'Cliente registrado') {
        res.render("cliente", {
            tipo: user.tipo,
            alert: true,
            alertTitle: "Registrado Correctamente",
            alertMessage: saveClient,
            icon: "success",
            timer: 1500,
            ruta: "cliente",
        })
    ...
    if (user.pass2 == user.pass) {
        ...
        if (usuario[0]) {
            ...
            if (updCliente === 'Cliente Atualizado') {
                ...
                res.render("login", {
                    alert: true,
                    alertTitle: "Ha habido un error al crear el usuario",
                    alertMessage: updCliente,
                    icon: "error",
                    timer: 1700,
                    ruta: "",
            ...
            res.render("login", {
                alert: true,
                alertTitle: 'El usuario no ha sido registrado',
                alertMessage: 'Póngase en contacto con la tienda para pedir su registro',
                icon: "error",
                timer: 2500,
                ruta: "",
            });
